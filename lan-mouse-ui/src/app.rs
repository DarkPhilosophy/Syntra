...
const APPLICATION_ID: &str = "de.feschber.LanMouse";
const APPLICATION_VERSION: &str = env!("SYNTRA_APPLICATION_VERSION");
...
/// Runs a standalone window. Production frontends should prefer a transport entry point.
pub fn run() -> Result<(), slint::PlatformError> {
    let app = create_app()?;
    #[cfg(not(target_os = "android"))]
    let Some(_instance) = acquire_frontend_instance(&app)
        .map_err(|error| slint::PlatformError::Other(error.to_string()))?
    else {
        return Ok(());
    };
    let state = Arc::new(Mutex::new(AppViewState::default()));
    let settings = Arc::new(Mutex::new(load_presentation_settings().unwrap_or_default()));
    initialize_presentation(&app, &state, Arc::clone(&settings));
    bind_window_callbacks(&app, settings, state, false);
    let _launch_ready_timer = schedule_launch_ready(&app);
    app.run()
}

/// Runs the Slint frontend against the daemon IPC connection.
pub fn run_with_ipc(
    reader: lan_mouse_ipc::FrontendEventReader,
                ...
                if received_history_page {
                    if let Some(page) = snapshot.history_page.as_ref() {
                        for record in &page.records {
                            ...
                            if record.kind == lan_mouse_ipc::HistoryKind::Image
...
struct IpcEventSource {
    reader: Mutex<lan_mouse_ipc::FrontendEventReader>,
...
impl EventSource for IpcEventSource {
    fn recv(&self) -> Result<FrontendEvent, TransportError> {
        ...
        let result = reader
            ...
            .map_err(|error| {
                log::error!("frontend IPC event failed: {error}");
    ...
    fn reconnect(&self) -> bool {
        let Ok((reader, mut writer)) = lan_mouse_ipc::connect() else {
            return false;
        };
...
fn local_device_profile(
...
) -> Result<lan_mouse_ipc::DeviceProfile, String> {
    ...
    let avatar = if settings.local_device.image_path.is_empty() {
        ...
        Some(lan_mouse_ipc::PeerAvatar {
            width: scaled.width(),
            height: scaled.height(),
            rgba: scaled.into_raw(),
        })
    };
    let profile = lan_mouse_ipc::DeviceProfile {
        display_name,
        avatar,
    };
        ...
        records
            ...
            .map(|record| {
                ...
                let (kind, preview) = match &record.preview {
                    lan_mouse_ipc::HistoryPreview::Text { preview, .. } => {
                        ("clipboard-text", preview.clone())
                    }
                    lan_mouse_ipc::HistoryPreview::Image { size_bytes, .. } => {
                        ("clipboard-images", format_bytes(*size_bytes))
                    }
                    lan_mouse_ipc::HistoryPreview::Files {
                        names,
                        total_size_bytes,
                        ..
                    } => (
                    ...
                    match action.as_str() {
                        ...
                        "start" => {
                            ...
                            if was_connected && !service::query()?.running {
                                ...
                                let socket = lan_mouse_ipc::default_socket_path()
                                    .map_err(|error| service::ServiceError::InvalidResponse(error.to_string()))?;
